 ...
 */
public class KnapsackPopulation implements Population<BitChromosome> {

    private List<BitChromosome> population;

    private KnapsackEncoder encoder;

    private Random random;

    public KnapsackPopulation() {
        this.encoder = new KnapsackEncoder();
        try {
    ...
    public List<BitChromosome> createPopulation(int size) {
        ...
        for (int i = 0; i < size; i++) {
            BitChromosome bitChromosome = encoder.createRandomChromosome(random);
